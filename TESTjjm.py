...
import time
class TESTjjm(StrategyTemplate):
    ...
    def trade_1(self):
        self.open_one = self.buy(self.leg1_symbol,self.main_ask_price,1)
        self.open_two = self.short(self.leg2_symbol,self.second_bid_price,2)
    ...
    def trade_2(self):
        self.open_two = self.buy(self.leg2_symbol,self.second_ask_price,2)
        self.open_one = self.short(self.leg1_symbol,self.main_bid_price,1)
    ...
    def closing_1(self):
        self.close_one = self.sell(self.leg1_symbol,self.main_bid_price,1)
        self.close_two = self.cover(self.leg2_symbol,self.second_ask_price,2)
    ...
    def closing_2(self):
        self.close_two = self.sell(self.leg2_symbol,self.second_bid_price,2)
        self.close_one = self.cover(self.leg1_symbol,self.main_ask_price,1)
    ...
    def track_open(self):
        ...
        elif (self.status_1.status in self.not_active) and (self.status_2.status in self.active) and (self.count - self.open_time  > 2400 or abs(self.open_price2 - (self.price2/1.2))>10):
    ...
    def send_open(self):
        ...
        if (self.status_1.status in self.active) and (self.status_2.status in self.not_active):
            ...
            if self.status_1.direction==Direction.LONG:
                ...
                if self.active_orderids == set() and self.get_pos(self.leg1_symbol)!=0:    
                    self.open_two = self.cover(self.leg1_symbol,self.main_ask_price,1)
                if num > 0:
                    if self.active_orderids == set() and self.get_pos(self.leg2_symbol)!=0:    
            ...
            if self.status_2.direction==Direction.LONG:
                if self.active_orderids == set() and self.get_pos(self.leg2_symbol)!=0:    
                    self.open_one = self.sell(self.leg2_symbol,self.second_bid_price,2)
                if self.tracked == True:
                    self.open_position_2 -= 1
                    self.tracked = False
            else:
                if self.active_orderids == set() and self.get_pos(self.leg2_symbol)!=0:    
                    self.open_one = self.cover(self.leg2_symbol,self.second_ask_price,2)
                if self.tracked == True:
                    self.open_position_1 -= 1
                    self.tracked = False
            self.send_open_time = self.count
        ...
        if tick.bid_price_1 > 1000000 or tick.bid_price_1 < 10:
            ...
            if 90000 <= self.tick_time and self.flag == False and self.waiting_open == False and self.waiting_close == False and self.running == True:
                ...
                if tick.vt_symbol == self.leg2_symbol:
                    ...
                    self.deal_price2 = (tick.bid_price_1  + tick.ask_price_1) * 1.2 / 2
            ...
            # self.price1 = np.log((tick.ask_price_1 + tick.bid_price_1) / 2)
                ...
                self.price2 = 1.2*(tick.ask_price_1 + tick.bid_price_1) / 2
                    ...
                    if self.main_ask_bid+self.second_ask_bid < 3:
                        ...
                        if self.open_position_1 > 0:
                            if 7200 < self.count-self.open_time <= 14400:# or (self.resid2 > (3.4/self.std)): 
                                if (self.main_bid_price-self.open_a1)*100-(self.second_ask_price-self.open_b2)*2*60 >= 120:
                                    self.closing_1()
                        ...
                        if self.open_position_2 > 0:
                            if 7200 < self.count-self.open_time < 14400:
                                if (self.second_bid_price-self.open_a2)*2*60-(self.main_ask_price-self.open_b1)*100 >= 120:
                                    self.closing_2()
