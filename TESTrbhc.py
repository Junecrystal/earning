...
import time
class TESTrbhc(StrategyTemplate):

    author = "用Python的交易员"
    leg1_symbol = ""
    leg2_symbol = ""
    variables = ["leg1_symbol", "leg2_symbol", "open_position_1", "open_position_2", "waiting_open", "waiting_close", "flag", "flag2", "complusary_closing_time", "close_one", "close_two","open_one","open_two","profit"]
    ...
    def __init__(
        ...
        #储存建模所用数据
        self.main_price, self.second_price= [0], [0]
    ...
    def trade_1(self,amount):
        self.open_one = self.buy(self.leg1_symbol,self.main_ask_price,amount)
        self.open_two = self.short(self.leg2_symbol,self.second_bid_price,amount)
    ...
    def trade_2(self,amount):
        self.open_two = self.buy(self.leg2_symbol,self.second_ask_price,amount)
        self.open_one = self.short(self.leg1_symbol,self.main_bid_price,amount)
    ...
    def closing_1(self,amount):
        self.close_one = self.sell(self.leg1_symbol,self.main_bid_price,amount)
        self.close_two = self.cover(self.leg2_symbol,self.second_ask_price,amount)
    ...
    def closing_2(self,amount):
        self.close_two = self.sell(self.leg2_symbol,self.second_bid_price,amount)
        self.close_one = self.cover(self.leg1_symbol,self.main_ask_price,amount)
    ...
    def track_open(self):
        ...
        if (self.status_1.status in self.active) and (self.status_2.status in self.not_active) and (self.count - self.open_time  > 2400 or abs(self.open_price1 - self.price1)>10):
            ...
            if self.status_2.direction == Direction.LONG:
                self.profit -=self.status_2.price
                self.profit +=self.status_1.price
            else:
                self.profit +=self.status_2.price
                self.profit -=self.status_1.price
    ...
    def send_open(self):
        ...
        if (self.status_1.status in self.active) and (self.status_2.status in self.not_active):
            ...
            untraded_1 = self.status_1.volume-self.status_1.traded
            untraded_2 = self.status_2.volume-self.status_2.traded
    ...
    def track_close(self):
        ...
        if (self.status_1.status in self.active) and (self.status_2.status in self.not_active) and (self.count - self.close_time  > 2400 or abs(self.close_price1 - self.price1)>10):
            ...
            self.write_log(['订单一成交'])
        elif (self.status_1.status in self.not_active) and (self.status_2.status in self.active) and (self.count - self.close_time  > 2400 or abs(self.close_price2 - self.price2)>10):
        ...
        if tick.bid_price_1 > 1000000 or tick.bid_price_1 < 10:
            ...
            if 90000 <= self.tick_time and self.flag == False and self.waiting_open == False and self.waiting_close == False and self.running == True:
                if tick.vt_symbol == self.leg1_symbol:
                    ...
                    self.deal_price1 =(tick.bid_price_1 + tick.ask_price_1) / 2
            ...
            if -0.5<=self.second_time-self.main_time<=0.5 and self.running == True:
                if self.flag == True and self.waiting_open == False and self.waiting_close == False:
                    ...
                    if self.main_ask_bid+self.second_ask_bid < 3:
                        if self.resid2 < -8/self.std and self.open_position_2 == 0 and self.open_position_1 == 0 and self.tick_time <=  self.complusary_open_time:
                            self.trade_2(10)
                        if self.open_position_2 > 0:
                            if self.resid2 - self.open_resid_2 > (10/self.std):
                                self.closing_2(self.open_position_2)    
                        if self.resid2 > (8/self.std) and self.open_position_1 == 0 and self.open_position_2 == 0 and self.tick_time <=  self.complusary_open_time:
                            self.trade_1(10)
                        if self.open_position_1 > 0:
                            if self.open_resid_1 - self.resid2 > (10/self.std):
                                self.closing_1(self.open_position_1)
                        if self.open_position_1 > 0:
                            if 14400 < self.count-self.open_time <= 28800:# or (self.resid2 > (3.4/self.std)): 
                                if (self.main_bid_price-self.open_a1)*10-(self.second_ask_price-self.open_b2)*10 >= 12:
                                    self.closing_1(self.open_position_1)
                            ...
                            if self.count-self.open_time > 28800:
                                ...
                                if self.flag2 == True:
                                    ...
                                    self.main_price.append(0)
                                    self.second_price.append(0)
                                else:self.running = False
                        if self.open_position_2 > 0:
                            if 14400 < self.count-self.open_time < 28800:
                                if (self.second_bid_price-self.open_a2)*10-(self.main_ask_price-self.open_b1)*10 >= 12:
                                    self.closing_2(self.open_position_2)
